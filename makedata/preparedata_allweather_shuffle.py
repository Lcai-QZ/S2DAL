...
parser.add_argument('--train_path', type=str, default='/media/zyserver/data16t/cailei/data/weather/allweather/allweather_train_mat_128',
...
new_order = np.random.permutation(allweather_data.shape[2])
allweather_data_rerange = allweather_data[:, :, new_order, :, :]
allweather_gt_rerange = allweather_gt[:, :, new_order, :, :]


# 1) 构造“未打乱”的帧标签（等长块：3 类）
frame_labels = make_frame_labels_equal_blocks(allweather_data, num_classes=3)  # shape: [T], dtype: long, device 同 allweather_data

# 2) 与数据使用相同的 new_order 打乱标签
frame_labels_shuffled = apply_same_shuffle_1d(frame_labels, new_order)  # shape: [T]

# 3) 可选：定义类别名映射（按你自己的类别顺序）
id2name = {0: "雨滴", 1: "雪花", 2: "雨雾"}
tracker = FrameCategoryTracker(frame_labels_shuffled, id2name=id2name)

for ss in range(ceil(allweather_img_num / args.batch_size)):
