...
# 构建网络
print('Loading from {:s}'.format(str(Path('/media/zyserver/data16t/cailei/project_SAHistorFomer/SAHistoFormer_V2/models_save_allweather_text_guided_degration_more_0.5_with_ssim/model_best_ssim_rainhaze8.pt'))))
model = define_network(deepcopy(opt['network_g'])).cuda()
# model = DerainNet(n_features=32, n_resblocks=8).cuda()
state_dict = torch.load(str(Path('/media/zyserver/data16t/cailei/project_SAHistorFomer/SAHistoFormer_V2/models_save_allweather_text_guided_degration_more_0.5_with_ssim/model_best_ssim_rainhaze8.pt')))
model.load_state_dict(state_dict)
model.eval()

# 加载数据
base_data_path = Path('/media/zyserver/data16t/cailei/data/weather/OutdoorRain/Test1')
...
rain_dir = Path('/media/zyserver/data16t/cailei/data/weather/OutdoorRain/Test1/input')
gt_dir = Path('/media/zyserver/data16t/cailei/data/weather/OutdoorRain/Test1/gt')
...
for kk, current_data in enumerate(current_data_list):
    ...
    for ii in range(ceil(num_frame / truncate_test)):
        ...
        img_id = np.full(inputs.shape[0], 2)  ## for rainhaze id
        ...
        with torch.set_grad_enabled(False):
            ...
            save_out.save(os.path.join(args.save_path, 'outputs', str(num)+'_derain.png'))
            save_input = transforms.ToPILImage()(inputs.squeeze().cpu())
            save_input.save(os.path.join(args.save_path, 'inputs', str(num) + '_rain.png'))
    ...
    if kk == 0:
        ...
        print('Type:Rainhaze, PSNR:{:5.2f}, SSIM:{:6.4f}'.format(psnrm_y, ssimm_y))
