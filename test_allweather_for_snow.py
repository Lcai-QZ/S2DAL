...
parser.add_argument('-opt', type=str, default='/media/zyserver/data16t/cailei/project_SAHistorFomer/SAHistoFormer_V2/Allweather/Options/Allweather_Histoformer_Degradation_Aware.yml', help='Path to option YAML file.')
parser.add_argument('--save_path', type=str, default='/media/zyserver/data16t/cailei/project_SAHistorFomer/SAHistoFormer_V2/experiments_allweather', help='save_path')
...
# 构建网络
print('Loading from {:s}'.format(str(Path('/media/zyserver/data16t/cailei/project_SAHistorFomer/SAHistoFormer_V2/models_save_allweather_text_guided_degration_more_0.5_with_ssim/model_best_ssim_snow_S11.pt'))))
model = define_network(deepcopy(opt['network_g'])).cuda()
# model = DerainNet(n_features=32, n_resblocks=8).cuda()
state_dict = torch.load(str(Path('/media/zyserver/data16t/cailei/project_SAHistorFomer/SAHistoFormer_V2/models_save_allweather_text_guided_degration_more_0.5_with_ssim/model_best_ssim_snow_S11.pt')))
model.load_state_dict(state_dict)
model.eval()

# 加载数据
base_data_path = Path('/media/zyserver/data16t/cailei/data/weather/Snow100K/Snow100K-S-10000-14999')
...
rain_dir = Path('/media/zyserver/data16t/cailei/data/weather/Snow100K/Snow100K-S-10000-14999/synthetic')
gt_dir = Path('/media/zyserver/data16t/cailei/data/weather/Snow100K/Snow100K-S-10000-14999/gt')
...
for ii, im_rain_path in enumerate(im_rain_path_list):
    ...
    im_rain = cv2.imread(str(im_rain_path), cv2.IMREAD_COLOR)
    im_rain = cv2.resize(im_rain, (target_w, target_h))  # (width, height)
    im_rain = im_rain[:, :, ::-1]
    im_rain = im_rain.transpose(2, 0, 1)

    im_rain_name = im_rain_path.name
    # im_gt_path = gt_dir / im_rain_name.replace('rain', 'clean')
    im_gt_path = gt_dir / im_rain_name
    # im_gt = img_as_float32(cv2.imread(str(im_gt_path), flags=cv2.IMREAD_COLOR)[:, :, ::-1].transpose([2, 0, 1]))
    im_gt = cv2.imread(str(im_gt_path), cv2.IMREAD_COLOR)
    im_gt = cv2.resize(im_gt, (target_w, target_h))  # (width, height)
    im_gt = im_gt[:, :, ::-1]
...
for kk, current_data in enumerate(current_data_list):
    ...
    for ii in range(ceil(num_frame / truncate_test)):
        ...
        with torch.set_grad_enabled(False):
            ...
            save_out.save(os.path.join(args.save_path, 'outputs', str(num)+'_desnow.png'))
            save_input = transforms.ToPILImage()(inputs.squeeze().cpu())
            save_input.save(os.path.join(args.save_path, 'inputs', str(num) + '_Snow100k-S.png'))
